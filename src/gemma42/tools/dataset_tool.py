...
class DatasetAppendTool(Tool):
    ...
    def run(self, args: dict, state: "AgentState") -> ToolResult:
        ...
        # Auto-id any row that's missing one. Without this, downstream tools
        # (extract_items, dataset_from_queue dedupe) silently break because
        # the silver dataset keys on `id`. Uses a sha1 of the row content so
        # the same row gets the same id on re-append.
