            ...
            if items and not items_have_ids:
                ...
                for it in items:
                    if not isinstance(it, dict):
                        continue
                    sig_src = json.dumps(
                        {k: it.get(k) for k in sorted(it.keys())},
                        default=str, ensure_ascii=False,
                    )
                    it["id"] = "sig_" + _hashlib.sha1(
                        sig_src.encode("utf-8")
                    ).hexdigest()[:12]
                items_have_ids = True
            new_items = []
            for it in items:
...
def _harvest_assets_for_item(
    *,
    ...
    from gemma42.tools.discover_assets_tool import _HREF_RE, _strip_tags, _classify
