...
class SetPlanTool(Tool):
    ...
    def run(self, args: dict, state: "AgentState") -> ToolResult:
        ...
        # CONTRACT RECONCILIATION: if the agent's observed `target_rows` is
        # below the standing MinRowsContract, that contract is unreachable —
        # it will keep the phase machine fighting for rows that don't exist.
        # Auto-downgrade the contract to match the observed reality and
        # surface the change to the user. This is conservative: we never
        # raise the contract, only lower it.
        from gemma_miner.contracts import MinRowsContract

        reconciled_note: str | None = None
        if isinstance(plan.get("target_rows"), int):
            for c in list(state.contracts.list()):
                if isinstance(c, MinRowsContract):
                    if plan["target_rows"] < c.min_rows:
                        old = c.min_rows
                        # ContractBook.add() replaces by name; safe to call.
                        state.contracts.add(
                            MinRowsContract(min_rows=int(plan["target_rows"]))
                        )
                        reconciled_note = (
                            f"min_rows contract auto-downgraded {old} → "
                            f"{plan['target_rows']} (plan target is the "
                            "achievable ceiling for this source)"
                        )
                        # Mirror the change in the plan's notes so users see why.
                        existing_notes = plan.get("notes") or ""
                        prefix = (existing_notes + " · ") if existing_notes else ""
                        plan["notes"] = prefix + reconciled_note
                    break
        ...
        if reconciled_note:
            lines.append("")
            lines.append(f"  ⚙  {reconciled_note}")
